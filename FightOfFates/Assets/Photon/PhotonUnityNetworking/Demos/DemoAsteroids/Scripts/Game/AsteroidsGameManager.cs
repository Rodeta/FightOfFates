...
namespace Photon.Pun.Demo.Asteroids
{
    public class AsteroidsGameManager : MonoBehaviourPunCallbacks
    {
        ...
        public void Start()
        {
            ...
            {
                {PlayAttribut.PLAYER_LOADED_LEVEL, true}
            };
        ...
        private IEnumerator SpawnAsteroid()
        {
            while (true)
            {
                yield return new WaitForSeconds(Random.Range(PlayAttribut.ASTEROIDS_MIN_SPAWN_TIME, PlayAttribut.ASTEROIDS_MAX_SPAWN_TIME));
        ...
        {
            if (changedProps.ContainsKey(PlayAttribut.PLAYER_LIVES))
            {
                CheckEndOfGame();
                return;
            }
            ...
            if (changedProps.ContainsKey(PlayAttribut.PLAYER_LOADED_LEVEL))
            {
        ...
        private bool CheckAllPlayerLoadedLevel()
        {
            foreach (Player p in PhotonNetwork.PlayerList)
            {
                ...
                if (p.CustomProperties.TryGetValue(PlayAttribut.PLAYER_LOADED_LEVEL, out playerLoadedLevel))
                {
        ...
        private void CheckEndOfGame()
        {
            ...
            {
                object lives;
                if (p.CustomProperties.TryGetValue(PlayAttribut.PLAYER_LIVES, out lives))
                {
                    if ((int) lives > 0)
                    {
                        allDestroyed = false;
